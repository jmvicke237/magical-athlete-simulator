...
class Gloth(Character):
    ...
    def modify_other_roll(self, other_player, game, play_by_play_lines, roll):
        ...
        self.register_ability_use(game, play_by_play_lines, description="Gloth")
        return new_roll

    def on_race_end(self, game, play_by_play_lines):
        # Eliminated Gloth still gets the bonus if they're at/before the corner —
        ...
        if self.position <= game.board.corner_position:
            ...
            self.register_ability_use(
                game, play_by_play_lines, description="Gloth end-of-race bonus"
            )
