...
class HighRoller(Character):
    ...
    Simulator policy: HighRoller chooses when to stop pressing their luck
    using the game.highroller_threshold knob (default 8). They keep rolling
    ...
    def main_roll(self, game, play_by_play_lines):
        ...
        while True:
            ...
            if busted:
                play_by_play_lines.append(
                    f"  HighRoller busts and doesn't move this turn!"
                )
                self.skip_main_move = True
                self.last_roll = 0
                self.register_ability_use(game, play_by_play_lines, description="HighRoller busted")
                return 0
